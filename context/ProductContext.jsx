...
export default function ProductsProvider(props) {
  ...
  function handleProduct(price_id, num, data, noIncrement = false) {
    const newCart = {
      ...cart,
    };
    if (price_id in cart) {
    ...
    if (parseInt(newCart[price_id].quantity) <= 0) {
      // the user has set the number to 0, so we need to remove product from the cart.
      delete newCart[price_id];
    }

    // overwrite the cart the state with new cart state.
    setCart(newCart);
  }
